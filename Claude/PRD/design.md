...
_Keep this file under 150 lines. When updating, refine — don't just add._
...
--color-accent:    #0072E5   /* Interaction only: links, highlights, hover states */
```

**Rules:**

- Accent appears on: hover states, the "In Their Eyes" text highlight, active nav indicator, the Ask Martta button.
...
**Switzer** — UI, headlines, navigation, card titles. Anything the user reads as interface.
**Geist Sans** — Body prose, captions, footer. Anything the user reads as content.

```
Display (Hero headline):  Switzer, clamp(2rem, 4vw, 3.5rem), weight 500, line-height 1.1
Section heading:          Switzer, 1.25rem, weight 500, line-height 1.3
Card title:               Switzer, 1rem, weight 500
Body / narrative:         Geist Sans, 1rem, weight 400, line-height 1.7
Caption / meta:           Geist Sans, 0.875rem, weight 400, color --color-muted
Nav:                      Switzer, 0.875rem, weight 500, letter-spacing 0.01em
```
...
Page horizontal pad:   clamp(1.5rem, 5vw, 5rem)
Section gap:           5rem–7rem vertical
Card internal pad:     1.5rem
Grid gap:              1.5rem
```

**The large breakpoint is 1200px.** Below it: single column or reduced columns.And a graceful and acceessible mobile view.

**Rules:**

- Cards should never feel crowded. If a grid looks tight, reduce columns before reducing padding.
- Negative space is a design element, not wasted space. The hero's empty column is intentional.
...
- The Ask Martta drawer slides in from the right. It does not fade in — it arrives.

---

## 7. Component Voice

**Project Cards:** The thumbnail does the talking. Title and description are supporting cast — one line each, no more.
...
**Ask Martta Drawer:** Functional and calm. Chat bubbles are minimal. This is a tool, not a personality showcase.

**Navigation:** Invisible until needed. Sticky but unobtrusive — it should never compete with the work below it.
...
- Descriptions of work answer: _what was the problem, what changed because of the design_ — not what tools were used.
...
Good update prompt for AI: _"We just decided [X]. Update design.md to reflect this, remove anything now redundant, and keep the file under 150 lines."_
